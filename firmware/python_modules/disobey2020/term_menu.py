...
class UartMenu():
	...
	def opt_launcher(self):
		system.start("dashboard.launcher", True)
	...
	def menu_settings(self):
		items = ["Change nickname", "Change picture", "WiFi configuration", "Change display orientation", "Homescreen configuration", "Calibrate touch buttons", "Update firmware", "< Return to main menu"]
		callbacks = [self.opt_change_nickname, self.opt_configure_picture, self.opt_configure_wifi, self.opt_configure_orientation, self.opt_configure_homescreen, self.opt_configure_calibrate, self.opt_ota, self.menu_main, self.menu_main]

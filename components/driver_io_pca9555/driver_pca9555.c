...
void driver_pca9555_intr_task(void *arg)
{
    ...
    while (1) {
        if (xSemaphoreTake(driver_pca9555_intr_trigger, portMAX_DELAY)) {
            ...
            for (int i = 0; i < sizeof(driver_pca9555_handler); i++) {
                if ((current_state & (1 << i)) != (previous_state & (1 << i))) {
                    ...
                    if (handler != NULL) {
                        handler(i, value);
                        printf("pca9555 handled %u\n", i); 
                    } else {
                    printf("pca9555 unhandled %u\n", i); 
                    }
                }
            }
...
{
    if (pin >= sizeof(driver_pca9555_handler)) return;
    printf("pca9555 set interrupt handler %u\n", pin);

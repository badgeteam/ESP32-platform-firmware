...
class LIS3DH:
    ...
    def shake(self, shake_threshold=30, avg_count=10, total_delay=0.1):
        ...
        for _ in range(avg_count):
            ...
            shake_accel[0] += abs(accel.x)
            shake_accel[1] += abs(accel.y)
            shake_accel[2] += abs(accel.z)
            time.sleep(total_delay / avg_count)
        avg = tuple(value / avg_count for value in shake_accel)
        total_accel = math.sqrt(sum(map(lambda x: x ** 2, avg)))
...
class LIS3DH_I2C(LIS3DH):
    ...
    def __init__(self, i2c, *args, address=0x18, int1=None, int2=None):
